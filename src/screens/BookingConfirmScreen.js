...
// Importar contextos, servicios y utilidades
...
export default function BookingConfirmScreen({ route, navigation }) {
  const { service, selectedService, selectedDate, selectedTime, isRescheduling, bookingId } = route.params
  ...
  const [totalPrice, setTotalPrice] = useState(selectedService.price)
  ...
  useEffect(() => {
    const calculateTotal = async () => {
      try {
        const total = await bookingService.calculateTotalPrice(service.id, {
          includeTax: true,
        })
        setTotalPrice(total)
      } catch (err) {
        console.error("Error calculating total price:", err)
        // Usar el precio del servicio como fallback
        setTotalPrice(selectedService.price)
      }
    }

    calculateTotal()
  }, [service.id, selectedService.price])
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (isRescheduling && bookingId) {
        ...
        result = await rescheduleBooking(bookingId, {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.detailRow}>
            ...
            <View style={styles.detailTextContainer}>
              ...
              <Text style={styles.detailSubtitle}>{dateUtils.formatDate(selectedDate)}</Text>
            </View>
          </View>
          <View style={styles.detailRow}>
            ...
            <View style={styles.detailTextContainer}>
              ...
              <Text style={styles.detailSubtitle}>{dateUtils.formatTime(selectedTime)}</Text>
            </View>
          </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.priceRow}>
            ...
            <Text style={styles.priceValue}>${(totalPrice - selectedService.price).toFixed(2)}</Text>
          </View>
          <View style={styles.priceRow}>
        ...
        <View style={styles.buttonContainer}>
          {isLoading ? (
            ...
            <TouchableOpacity style={styles.confirmButton} onPress={handleConfirm}>
              <Text style={styles.confirmButtonText}>{isRescheduling ? "Confirmar cambios" : "Confirmar y pagar"}</Text>
            </TouchableOpacity>
          )}
