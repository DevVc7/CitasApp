...
export default function BookingSuccessScreen({ route, navigation }) {
  const { booking, service, selectedService, selectedDate, selectedTime, isRescheduling } = route.params
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.successContainer}>
          ...
          <Text style={styles.successTitle}>{isRescheduling ? "¡Cita reprogramada!" : "¡Reserva confirmada!"}</Text>
          <Text style={styles.successMessage}>
            {isRescheduling ? "Tu cita ha sido reprogramada exitosamente." : "Tu cita ha sido reservada exitosamente."}
        ...
        <View style={styles.card}>
          ...
          <View style={styles.priceRow}>
            ...
            <Text style={styles.priceValue}>${(selectedService.price * 0.16).toFixed(2)}</Text>
          </View>
          <View style={styles.priceRow}>
            <Text style={styles.totalLabel}>Total</Text>
            <Text style={styles.totalValue}>${(selectedService.price * 1.16).toFixed(2)}</Text>
          </View>
        </View>
